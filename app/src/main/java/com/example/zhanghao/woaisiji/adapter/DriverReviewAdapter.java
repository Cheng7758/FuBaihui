...
import com.example.zhanghao.woaisiji.adapter.viewholder.DriverReviewHolder;
...
import com.example.zhanghao.woaisiji.adapter.viewholder.NewImageViewHolder;
import com.example.zhanghao.woaisiji.adapter.viewholder.NewURLViewHolder;
import com.example.zhanghao.woaisiji.adapter.viewholder.NewVideoViewHolder;
...
import com.example.zhanghao.woaisiji.widget.dialog.NewCommentDialog;
...
public class DriverReviewAdapter extends BaseRecycleViewAdapter {
    ...
    private DriverReviewPresenter presenter;
    private Context context;
    private String favortNum;
    private AlterResultBean resultBean;
    private String circleId;

    public void setCirclePresenter(DriverReviewPresenter presenter){
...
//                    ImageLoader.getInstance().displayImage(data,holder.rivDynamicHeadPic);
            ...
            //发布
//            holder.btnDynamicRelease.setOnClickListener(new View.OnClickListener() {
//                @Override
//                public void onClick(View v) {
//                    context.startActivity(new Intent(context, ReleaseDynamicActivity.class));//跳转到写动态界面
//                }
//            });

//                setHeadPicture(holder.rivDynamicHeadPic);
            ...
            final DriverReviewHolder holder = (DriverReviewHolder) viewHolder;
                    ...
                    holder.commentList.setOnItemClickListener(new CommentListView.OnItemClickListener() {
                        ...
                        public void onItemClick(int commentPosition) {
                            ...
                            if((WoAiSiJiApp.getCurrentUserInfo()!=null)&&(WoAiSiJiApp.getCurrentUserInfo().getPic().equals(commentItem.getUser().getId()))){//复制或者删除自己的评论
                                NewCommentDialog dialog = new NewCommentDialog(context, presenter, commentItem, circlePosition);
                                dialog.show();
                            }else{//回复别人的评论
                                if(presenter != null){
            ...
            switch (holder.viewType) {
                case CircleViewHolder.TYPE_URL:// 处理链接动态的链接内容和和图片
                    if(holder instanceof NewURLViewHolder){
                        String linkImg = circleItem.getLinkImg();
                        String linkTitle = circleItem.getLinkTitle();
                        Glide.with(context).load(linkImg).into(((NewURLViewHolder)holder).urlImageIv);
                    }
                    break;
                case CircleViewHolder.TYPE_IMAGE:// 处理图片
                    if(holder instanceof NewImageViewHolder){
                        final List<String> photos = circleItem.getPhotos();
                        if (photos != null && photos.size() > 0) {
                            ((NewImageViewHolder)holder).multiImageView.setVisibility(View.VISIBLE);
                            ((NewImageViewHolder)holder).multiImageView.setList(photos);
                            ((NewImageViewHolder)holder).multiImageView.setOnItemClickListener(new MultiImageView.OnItemClickListener() {
                                @Override
                            ...
                            ((NewImageViewHolder)holder).multiImageView.setVisibility(View.GONE);
                        }
                    }

                    break;
                case CircleViewHolder.TYPE_VIDEO:
                    if(holder instanceof NewVideoViewHolder){
                        ((NewVideoViewHolder)holder).videoView.setVideoUrl(circleItem.getVideoUrl());
                        ((NewVideoViewHolder)holder).videoView.setVideoImgUrl(circleItem.getVideoImgUrl());//视频封面图片
                        ((NewVideoViewHolder)holder).videoView.setPostion(position);
                        ((NewVideoViewHolder)holder).videoView.setOnPlayClickListener(new CircleVideoView.OnPlayClickListener() {
                            @Override
